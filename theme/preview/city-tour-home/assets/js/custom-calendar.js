...
(function ($) {
  ...
  function initializeTourCalendar($tourCalendar) {
    ...
    function updateInputField(formattedDate) {
      ...
      const isBreadcrumbSearch = $tourDisplay.closest(".city-breadcrumb-search").length;
      const breadcrumbDate = moment(formattedDate).format("D MMMM, YYYY");

      if ($tourDateField.length) {
        $tourDateField.val(isBreadcrumbSearch ? breadcrumbDate : formattedDate);
      }

      if ($tourDisplay.length) {
        $tourDisplay.html(
          isBreadcrumbSearch
            ? `<strong class="ui-title">${breadcrumbDate}</strong>`
            : `<strong class="ui-title">${formattedLabel}</strong><span>${formattedMeta}</span>`,
        );
      }
    }
