...
(function ($) {
  ...
  function initializeTourCalendar($tourCalendar) {
    ...
    function getActiveDateField() {
      return $tourCalendar.closest(".date-field");
    }
    ...
    function updateInputField(formattedDate) {
      const formattedLabel = moment(formattedDate).format("D MMMM, YYYY");
      const formattedMeta = moment(formattedDate).format("dddd YYYY");
      const { $activeDateField, $tourDateField, $tourDisplay } = getActiveDateElements();
      const isBannerDateField = $activeDateField.hasClass("banner-date-field");
      ...
      if ($tourDisplay.length) {
        if (isBannerDateField) {
          $tourDisplay.html(`<strong class="ui-title">${formattedLabel}</strong>`);
        } else {
